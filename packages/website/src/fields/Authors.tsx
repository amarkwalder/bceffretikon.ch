    ...
    return (
        <>
            <AuthorsHeader>
                ...
                <IconButton primary small onClick={() => setVisible(!visible)} open={visible}>
                    <AddIcon />
                </IconButton>
...
const AuthorListItem: React.FC<AuthorListItemProps> = ({ author, form, field, index }) => {
        ...
        <Draggable key={index} draggableId={`${field.name}.${index}`} index={index}>
            {(provider, snapshot) => (
                ...
                >
                    <DragHandle />
                    <ItemLabel>
                        {author && author.name ? author.name : <Placeholder>Unknown Author</Placeholder>}
                    </ItemLabel>
...
const ItemLabel = styled.label<{ error?: boolean }>`
    margin: 0;
...
const AuthorMenu = styled.div<{ open: boolean }>`
    min-width: 12rem;
